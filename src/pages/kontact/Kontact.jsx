import React from 'react'

function Kontact() {
    return (
        <div className='kon'>
            <div className="contakt">
                <div className="container">
                    <iframe data-aos="fade-up" className='location'
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2250.588803794508!2d37.477642515926995!3d55.661360580528985!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x46b54da002656bc7%3A0xa799933657c1d4f!2z0L_RgC4g0JLQtdGA0L3QsNC00YHQutC-0LPQviwgODbQsiwg0JzQvtGB0LrQstCwLCAxMTk0MTU!5e0!3m2!1sru!2sru!4v1645970050041!5m2!1sru!2sru">
                    </iframe>
                    <div data-aos="fade-up" className="contakt-footer">
                        <h1>+998 94 105 57 99</h1>
                        <h2>Москва ул. Проспект Вернадского 86В ( Парк Фудхол)</h2>
                        <div>
                            <p>Доставка и самовывоз:</p>
                            <p>ПН-ЧТ - 10:00 -23:00</p>
                            <p>ПТ-СУБ - 10:00 -24:00</p>
                            <p>ВСК - 11:00 - 23:00</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Kontact